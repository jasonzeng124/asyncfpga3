...
// ---------------------------------------------------------------------------
// tb_smoke_collatz64_gen -- generic no-oracle UNIFORM-mode smoke test for the
...
// requires: build/gen/collatz64_bench_gen.v
// requires: build/gen/collatz64_kernel_bench.v

`include "build/gen/collatz64_bench_gen.v"
...
module tb_smoke_collatz64_gen;
    ...
    collatz64_bench_gen_bridge dut (
        ...
        for (t = 0; t < 2_000_000; t = t + 1) begin
            ...
            if (t % `SMOKE_HB == 0 && t != 0)
                $display("%s heartbeat: cycle=%0d st=%0d runs_done=%0d",
                         "tb_smoke_collatz64_gen", t, dut.st, dut.runs_done);
            if (dut.bench_done) begin bstatus = 1; t = 2_000_000; end
        end
        if (!bstatus) begin
            $display("collatz64 smoke STALL: st=%0d runs_done=%0d", dut.st, dut.runs_done);
            $display("tb_smoke_collatz64_gen FAIL"); $finish;
        end
        rd(LATMIN, latmin); rd(LATMAX, latmax);
        hsum = 0;
        for (hi = 0; hi < 64; hi = hi + 1) hsum = hsum + dut.hist[hi];
        $display("collatz64 smoke done: runs_done=%0d lat_min=%0d lat_max=%0d hist_sum=%0d",
                  dut.runs_done, latmin, latmax, hsum);
        if (dut.runs_done === 32'd`SMOKE_RUNS && hsum === 32'd`SMOKE_RUNS && latmin <= latmax)
            $display("tb_smoke_collatz64_gen PASS");
        else
            $display("tb_smoke_collatz64_gen FAIL");
        $finish;
    end
endmodule
