...
// ---------------------------------------------------------------------------
// tb_null_isprime_gen -- exercises the generated null-kernel control for isprime
...
//
// Run (after `python3 hw/gen_bench.py isprime --null --top isprime_null_bench_gen
// -o build/gen/isprime_null_bench_gen.v`, from cells/):
//   iverilog -g2012 -gspecify -Wall -Wno-timescale -s tb_null_isprime_gen \
//     -o build/sim/tb_null_isprime_gen.vvp \
//     sim/bd_prims_sim.v sim/bd_env.v rtl/*.v tb/tb_null_isprime_gen.v
//   vvp build/sim/tb_null_isprime_gen.vvp
// ---------------------------------------------------------------------------

// requires: sim/ps7_stub.v
// requires: build/gen/isprime_null_bench_gen.v

`include "build/gen/isprime_null_bench_gen.v"

module tb_null_isprime_gen;
    ...
    isprime_null_bench_gen_bridge dut (
    ...
    initial begin
        ...
        // isprime: single-arg kernel, OP1 unused (see gen_bench.py's fixed operand-word mapping)
        wr(NRUNS, 32'd50);
        ...
        if (!bstatus) begin
            ...
            $display("tb_null_isprime_gen FAIL"); $finish;
        end
        ...
        if (dut.runs_done === 32'd50 && hsum === 32'd50)
            $display("tb_null_isprime_gen PASS");
        else
            $display("tb_null_isprime_gen FAIL");
        $finish;
    end
endmodule
