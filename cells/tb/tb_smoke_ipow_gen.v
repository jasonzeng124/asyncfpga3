...
// ---------------------------------------------------------------------------
// tb_smoke_ipow_gen -- generic no-oracle UNIFORM-mode smoke test for the
...
// requires: build/gen/ipow_bench_gen.v
// requires: build/gen/ipow_kernel_bench.v

`include "build/gen/ipow_bench_gen.v"
        ...
        for (t = 0; t < 2_000_000; t = t + 1) begin
            ...
            if (t % `SMOKE_HB == 0 && t != 0)
                $display("%s heartbeat: cycle=%0d st=%0d runs_done=%0d",
                         "tb_smoke_ipow_gen", t, dut.st, dut.runs_done);
            if (dut.bench_done) begin bstatus = 1; t = 2_000_000; end
        end
        if (!bstatus) begin
            $display("ipow smoke STALL: st=%0d runs_done=%0d", dut.st, dut.runs_done);
            $display("tb_smoke_ipow_gen FAIL"); $finish;
        end
        rd(LATMIN, latmin); rd(LATMAX, latmax);
        hsum = 0;
        for (hi = 0; hi < 64; hi = hi + 1) hsum = hsum + dut.hist[hi];
        $display("ipow smoke done: runs_done=%0d lat_min=%0d lat_max=%0d hist_sum=%0d",
                  dut.runs_done, latmin, latmax, hsum);
        if (dut.runs_done === 32'd`SMOKE_RUNS && hsum === 32'd`SMOKE_RUNS && latmin <= latmax)
            $display("tb_smoke_ipow_gen PASS");
        else
            $display("tb_smoke_ipow_gen FAIL");
        $finish;
    end
endmodule
