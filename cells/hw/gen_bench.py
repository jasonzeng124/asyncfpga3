...
def emit(kernel, is_null, top_name):
    ...
    if is_null:
        ...
        P(f"    // N=80 here is load-bearing, not a stylistic delay-pad: the bench FSM's")
        P(f"    // S_WAIT_ACK step polls the RAW i_ack_pl wire (unsynchronized, on purpose,")
        P(f"    // so the synchronizer's own latency never contaminates a measured run --")
        P(f"    // see the bridge's o_req_s comment for the one place that IS synchronized).")
        P(f"    // A synchronous poller can only see a level that is HIGH AT a clock edge.")
        P(f"    // With a short self-ack chain (this was N=6), joined_req -> joined_ack -> ")
        P(f"    // ack_out -> the fork's own ack -> req_core's OWN async clear (req_clr)")
        P(f"    // round-trips in under ~1 ns of simulated delay -- the request rises, gets")
        P(f"    // acknowledged, and clears itself entirely BETWEEN two clock edges. That is")
        P(f"    // not a rare race: it is deterministic given a fixed clock phase, and it")
        P(f"    // hung the null-kernel control 100% of the time in simulation (confirmed by")
        P(f"    // tracing req_clr/i_ack_pl/req_core cycle-by-cycle: req_core pulses 1 then")
        P(f"    // 0 within the same ~1 ns window, never coinciding with a clock edge, so")
        P(f"    // S_WAIT_ACK's `if (i_ack_pl)` check never fires and the FSM never retries).")
        P(f"    // N=80 gives ~80*124 ps = ~9.9 ns per traversal at BD_ROUTE_PS=0, so the")
        P(f"    // round-trip HIGH duration of joined_ack/i_ack_pl is on the order of two")
        P(f"    // traversals (~20 ns) -- comfortably wider than one 10 ns (100 MHz) period,")
        P(f"    // and still wider than one 4 ns (250 MHz) period with margin. The real kernels")
        P(f"    // never hit this: their own compiled logic (tens to hundreds of matched")
        P(f"    // delays) is intrinsically slower than one clock period, so their ack pulse")
        P(f"    // was always wide enough by construction -- only the deliberately-trivial")
        P(f"    // null control was fast enough to fall through the gap.")
        P(f"    bd_delay #(.N(80)) uack (.a(joined_req), .z(joined_ack));")
        P(f"    bd_delay #(.N(80)) uout (.a(joined_req), .z(out0_req));")
        fold_terms = []
        for a in data_args:
    ...
    P("            runs_done <= 32'b0; lat_ctr <= 32'b0; prep_ctr <= 4'b0;")
    ...
    P("                        if (prep_ctr == 4'd15) begin")
    P("                            prep_ctr <= 4'd0; lat_ctr <= 32'b0; st <= S_ISSUE;")
    P("                        end")
    P("                    end")
    P("")
    P("                    S_ISSUE: begin bench_set_req <= 1'b1; st <= S_WAIT_ACK; end")
    P("                    S_WAIT_ACK: if (i_ack_pl) st <= S_WAIT_RES;")
    P("")
    P("                    S_WAIT_RES: if (o_req_s) begin")
    ...
    P("                            if (!have_ref) begin")
    P("                                have_ref <= 1'b1; mismatch_ref <= o_data_capture;")
    P("                            end else if (!mismatch_sticky && o_data_capture != mismatch_ref) begin")
    P("                                mismatch_sticky <= 1'b1;")
    P("                                mismatch_idx <= runs_done;")
    P("                                mismatch_val <= o_data_capture;")
    P("                            end")
    P("                        end")
    P("                        bench_o_ack <= 1'b1;")
    P("                        st <= S_ACK;")
