...
def emit(kernel, is_null, top_name):
    ...
    P("    reg [31:0] hist_rd;                    // stage C: hist[bucket_q], read mux alone")
    P("    reg [3:0]  lead1;")
    P("    reg        found1;")
    P("    reg [1:0]  mant;")
    P("    reg [5:0]  bucket;")
    ...
    P("    always @(posedge aclk or negedge aresetn) begin")
    P("        if (!aresetn) begin")
    P("            bucket_q <= 6\'b0; bucket_q2 <= 6\'b0; hist_rd <= 32\'b0;")
    P("            hist_v2  <= 1\'b0; hist_we <= 1\'b0;")
    P("        end else begin")
    ...
    P("            hist_v2   <= hist_pend;")
    P("            hist_rd   <= hist[bucket_q];    // C: 64:1 read mux -> reg")
    P("            bucket_q2 <= bucket_q;")
    P("            hist_we   <= hist_v2;           // D: add + write, next cycle")
    P("        end")
    P("    end")
    P("")
    P("    always @(posedge aclk or negedge aresetn) begin")
    ...
    P("                hist_pend <= 1'b0;")
    P("                // Stage D.  The batch-start clear in S_IDLE is later in source")
    P("                // order and therefore beats a straggler write, which is the")
    P("                // priority we want: a run retiring into a histogram that has")
    P("                // just been zeroed for a NEW batch must not be counted in it.")
    P("                if (hist_we && hist_rd != 32'hFFFFFFFF)")
