...
def emit(kernel, is_null, top_name):
    ...
    P("    always @(posedge aclk or posedge i_ack_pl or negedge aresetn) begin")
    P("        if (!aresetn)            i_ack_latched <= 1\'b0;")
    P("        else if (st == S_ISSUE)  i_ack_latched <= 1\'b0;   // clear once, at request launch")
    P("        else if (i_ack_pl)       i_ack_latched <= 1\'b1;   // async SET, armed all transaction")
    P("    end")
    P("    always @(posedge aclk or posedge o_req_pl or negedge aresetn) begin")
    P("        if (!aresetn)            o_req_latched <= 1\'b0;")
    P("        else if (st == S_ISSUE)  o_req_latched <= 1\'b0;")
    P("        else if (o_req_pl)       o_req_latched <= 1\'b1;")
    P("    end")
    P("")
    ...
    P("                        // Reads o_data_pl DIRECTLY, not the free-running")
    P("                        // o_data_capture register (still used by the ODATA")
    P("                        // readback for manual/host polling): o_data_capture")
    P("                        // trails o_data_pl by one clock cycle unconditionally,")
    P("                        // which was fine paired with the old 2-cycle o_req_s")
    P("                        // synchronizer (plenty of settling margin) but reads")
    P("                        // STALE data now that o_req_latched can fire one cycle")
    P("                        // earlier -- caught as a wrong SIG and a spurious FIXED-")
    P("                        // mode repeatability mismatch in tb_gcd_bench_gen.v.")
    P("                        // o_data_pl itself is safe here: it is combinational off")
    P("                        // operands held steady for the whole transaction (unlike")
    P("                        // the req/ack pulses), so it is already stable.")
    P("                        sig <= {sig[30:0], sig[31]} ^ o_data_pl;")
