...
# ---------------------------------------------------------------------------
# EVERY INPUT MUST FIT THE KERNEL'S COMPILED ARGUMENT WIDTH.  This is not a
# formality.  collatz's port is 16 bits wide -- Dynamatic narrows it from the
# main() that calls collatz(27), and the backend emits what it is handed -- so
# the first version of this sweep asked for 77031 and the hardware answered for
# 77031 & 0xFFFF = 11495, which really does take 86 steps.  A perfectly
# self-consistent wrong point, on both builds, that only the expected-value
# check caught.  META carries the width; read it and refuse rather than trust
...
# --- input-width probe -----------------------------------------------------
# 52527 computes correctly and its peak intermediate is 106358020 (27 bits), so
# the internal datapath is NOT narrow -- yet 77031 answered 86, which is exactly
# collatz(77031 & 0xFFFF) = collatz(11495).  Find the boundary instead of
# guessing.  Each input below is chosen so the TRUE answer and the 16-bit
# TRUNCATED answer differ, and whichever the board returns names the cause.
#
# Inputs whose low 16 bits are ZERO are deliberately excluded: collatz(0) never
# terminates (0 is even, 0>>1 is 0), so probing 65536 would wedge the board
# rather than answer anything.
puts ""
puts "input-width probe:"
puts "  input      board   true    trunc16  verdict"
foreach n {11495 65535 77031 142567 100000} true {86 130 350 82 129} tr {86 130 86 86 65} {
    set d [run_batch 1 1 $n 0]
    set got [dict get $d mism_ref]
    if {$got == $true} { set v "matches TRUE (full width)" \
    } elseif {$got == $tr} { set v "matches TRUNC16 -- input truncated" \
    } else { set v "matches NEITHER" }
    puts [format "  %-10d %-7d %-7d %-8d %s" $n $got $true $tr $v]
}
puts ""
