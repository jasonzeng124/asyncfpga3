...
# --- determinism probe -----------------------------------------------------
# Is a wrong answer STABLE?  A deterministic wrong value is broken logic --
# a mis-synthesised datapath, a wrong constant, a dropped bit.  A value that
# varies across identical transactions is a race: something is sampled before
# it settles, and no amount of staring at the netlist's truth tables will
# show it.  These are different bugs with different fixes, and one repeat
# loop separates them.
#
# ipow(1,16777215) is the sharpest vector here: every multiply it performs is
# 1*1, so the multiplier -- DSP or otherwise -- cannot contribute a wrong
# value.  If that one is wrong, the wrongness is not in the arithmetic.
set probe_vectors {
    {1  16777215}
    {7  11}
    {10 9}
...
set REPS 50
foreach v $probe_vectors {
    lassign $v a b
    set want [ipow_ref $a $b]
    set seen [dict create]
    set to 0
    for {set k 0} {$k < $REPS} {incr k} {
        if {[catch {ipow_once $a $b} got]} { incr to } \
        else { dict incr seen $got }
    }
    set distinct [llength [dict keys $seen]]
    set verdict [expr {$distinct == 1 && [dict exists $seen $want] ? "STABLE-CORRECT" \
                  : ($distinct == 1 ? "STABLE-WRONG" : "VARYING")}]
    puts "ipow($a,$b) want=$want reps=$REPS timeouts=$to distinct=$distinct  $verdict"
    foreach k [lsort -integer [dict keys $seen]] {
        puts [format "      %-12s x%-4d %s" $k [dict get $seen $k] \
              [expr {$k == $want ? "<- correct" : ""}]]
    }
}
puts "=== PROBE COMPLETE ==="
