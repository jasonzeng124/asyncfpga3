...
if {$bitfile eq ""} { set bitfile "build/hw/ipow_ps_dsp/ipow_ps.bit" }
...
# --- localisation scans ----------------------------------------------------
# The determinism probe showed the wrong answers are stable across 50 repeats,
# so this is logic, not a race.  These three scans separate the two things the
# kernel does -- iterate, and multiply -- by holding one of them trivial.
#
#   A  b=1, e swept.  Every multiply is 1*1, so ANY wrong answer here is the
#      loop's, not the multiplier's.  Expected 1 for every e.
#   B  e=2, b swept.  Exactly one squaring and one accumulate, the shortest
#      path through the multiplier.  Expected b*b mod 2**32.
#   C  b=2, e swept.  Exercises the accumulate across a growing trip count
#      with values whose bit patterns are trivially checkable.
proc scan {label vectors} {
    set bad {}
    foreach v $vectors {
        lassign $v a b
        set want [ipow_ref $a $b]
        if {[catch {ipow_once $a $b} got]} { set got "TIMEOUT" }
        if {$got ne $want} { lappend bad [list $a $b $got $want] }
    }
    puts "--- $label: [llength $vectors] vectors, [llength $bad] wrong"
    foreach t $bad {
        lassign $t a b got want
        puts [format "      b=%-12s e=%-10s got=%-12s want=%s" $a $b $got $want]
    }
    return [llength $bad]
}

set A {}
for {set e 0} {$e <= 40} {incr e} { lappend A [list 1 $e] }
scan "A  base=1, exponent 0..40 (multiplier cannot be at fault)" $A

set B {}
foreach b {0 1 2 3 4 5 7 10 15 16 17 255 256 257 65535 65536 65537 1000000 2863311530 4294967295} {
    lappend B [list $b 2]
}
scan "B  exponent=2, base swept (one squaring)" $B

set C {}
for {set e 0} {$e <= 40} {incr e} { lappend C [list 2 $e] }
scan "C  base=2, exponent 0..40" $C
