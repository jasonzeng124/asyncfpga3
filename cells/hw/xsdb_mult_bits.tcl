...
if {$bitfile eq ""} { set bitfile "build/hw/mult_ps_dsp_keep/mult_ps.bit" }
...
# --- bit-level characterisation -------------------------------------------
# The pass/fail count says the multiply is broken.  It does not say WHICH
# BITS are broken, and that is the question a 3-DSP cascade answers: each
# stage owns a 17-bit slice of the product, so a stage that is not
# contributing shows up as a contiguous band of wrong bits at a known
# offset.  got XOR want names that band directly.
proc mult_ref {a b} { return [expr {($a * $b) & 0xFFFFFFFF}] }
proc b32 {v} { return [format %032b $v] }

proc show {a b} {
    set want [mult_ref $a $b]
    if {[catch {mult_once $a $b} got]} { puts "  TIMEOUT"; return }
    set x [expr {$got ^ $want}]
    set lo -1
    set hi -1
    for {set i 0} {$i < 32} {incr i} {
        if {($x >> $i) & 1} { if {$lo < 0} { set lo $i }; set hi $i }
    }
    set tag [expr {$x == 0 ? "ok" : "wrong bits $lo..$hi"}]
    puts [format "  %10u * %-10u  got %s" $a $b [b32 $got]]
    puts [format "  %10s   %-10s want %s   %s" "" "" [b32 $want] $tag]
    puts [format "  %10s   %-10s xor  %s" "" "" [b32 $x]]
    puts ""
}

puts "=== powers of two: the product occupies ONE bit, so the band is exact"
foreach k {0 1 2 3 4 8 12 16 17 18 20 24 28 31} {
    show [expr {1 << $k}] 1
}
puts "=== 1 * (single bit): only the B operand moves"
foreach k {0 4 8 16 17 20 24 31} { show 1 [expr {1 << $k}] }
puts "=== small squares (these returned 0)"
foreach v {2 3 4 10 15 16 17 255 256} { show $v $v }
puts "=== SCAN COMPLETE ==="
exit 0
