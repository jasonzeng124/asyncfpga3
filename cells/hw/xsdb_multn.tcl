...
if {$bitfile eq ""} { set bitfile "build/hw/multn_ps/multn_ps.bit" }
...
# eight products, 0x10 .. 0x2C -- index k is XORed with k, see hw/multn_ps.v
set OBASE  [expr {$BASE + 0x10}]
...
# --- DSP48E1 site map ------------------------------------------------------
# Which product index sits on which site (from the routed netlist):
set site(0) DSP48_X0Y30
set site(1) DSP48_X0Y12
set site(2) DSP48_X0Y22
set site(3) DSP48_X0Y32
set site(4) DSP48_X0Y14
set site(5) DSP48_X0Y16
set site(6) DSP48_X0Y20
set site(7) DSP48_X0Y24
...
proc all8 {a b} {
    ...
    for {set k 0} {$k < 8} {incr k} {
        lappend r [expr {[mrd -value [expr {$::OBASE + 4*$k}]] & 0xFFFFFFFF}]
    }
    mwr -force $::CTRL 0x2
...
for {set k 0} {$k < 8} {incr k} { set ok($k) 0; set bad($k) 0; set zed($k) 0 }
expr {srand($SEED)}
set vecs {}
foreach x {1 2 3 7 255 4096 65535} { lappend vecs [list $x $x] }
...
foreach v $vecs {
    ...
    set got [all8 $a $b]
    for {set k 0} {$k < 8} {incr k} {
        set want [expr {((($a & 0xFFFF) ^ $k) * ($b & 0xFFFF)) & 0xFFFFFFFF}]
        set g [lindex $got $k]
        if {$g == $want} { incr ok($k) } else {
            incr bad($k)
            if {$g == 0} { incr zed($k) }
        }
    }
}
set total [llength $vecs]
puts ""
puts "site          product   pass   fail   of which zero"
for {set k 0} {$k < 8} {incr k} {
    puts [format "%-14s  %d      %5d  %5d   %5d" $site($k) $k $ok($k) $bad($k) $zed($k)]
}
puts "(total $total vectors)"
exit 0
