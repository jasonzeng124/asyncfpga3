...
# THIS LOOP IS THE THING build_bench.sh's DEFAULT PATH NOW CALLS, so it has to
# announce itself to the builds it drives.  build_bench.sh, with no BD_SIZES
# and no BD_NO_TIGHTEN, execs into this script; every build below then calls
# build_bench.sh again, and without this export each of those would take the
# same default branch and exec back into here.  The recursion is unbounded and
# each level costs a full place-and-route, so it would not look like a hang so
# much as a machine that never finishes.
#
# Exporting it here rather than prefixing each build covers the direct
...
# Distinguish "seeded" from "raised nothing".  The first merge adds every cell
# and raises none of them, and reporting that as "nothing raised" made gcd's
# run look like it had already converged when it had not yet started.
if not prev:
    msg = f"seeded {len(merged)} cell(s) from the first route"
elif raised:
    msg = ", ".join(raised) + f"  ({len(merged)} cell(s))"
else:
    msg = f"nothing raised  ({len(merged)} cell(s))"
print("   ratchet: " + msg)
RPY
    ...
    if ! BD_SIZES_GATE=0 BD_NO_TIGHTEN=${BD_SKIP_RULE_E:-0} \
         ./hw/build_bench.sh "$K" > "$HIST/build.$i.log" 2>&1; then
        echo "   BUILD FAILED with these sizes -- not a candidate"
        if grep -q "NOT CONVERGING" "$HIST/build.$i.log"; then
            echo "   (that is verify/converge.sh -- RULE E -- failing to settle,"
            echo "    not rule A.  Rule E pads select channels and its own"
            echo "    message says the margins are below this fabric's routing"
            echo "    noise, so it cannot hold a per-channel constant across"
            echo "    builds.  Re-run with BD_SKIP_RULE_E=1 to size rule A"
            echo "    without it, and pad selects separately afterwards.)"
        fi
