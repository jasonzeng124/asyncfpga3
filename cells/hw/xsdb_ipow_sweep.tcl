...
if {$bitfile eq ""} { set bitfile "build/hw/ipow_ps/ipow_ps.bit" }
...
for {set i 0} {$i < $N} {incr i} {
    # Base: the full 32 bits.  Wrapping is the behaviour under test, and the
    # multiply is sign-agnostic, so there is no reason to restrict it.
    set a [expr {int(rand() * 4294967296)}]
    # Exponent: draw the BIT LENGTH uniformly in [1,24], then a value of that
    # length.  Drawing the value uniformly in [0,2**24) would put almost every
    # vector at 24 loop iterations; this spreads the trip count, which is the
    # thing that actually varies the datapath's occupancy.
    set bits [expr {1 + int(rand() * 24)}]
    set b [expr {int(rand() * (1 << $bits))}]
    run_one $a $b
}
