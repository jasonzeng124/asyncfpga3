...
# --- bit-walk probe -------------------------------------------------------
# Which A bit / which B bit does each lone DSP actually respond to?
# a = 1<<k, b = 1  -> P must be 1<<k.   Then a = 1, b = 1<<k.
proc both {a b} {
    mwr -force $::ADATA $a
    mwr -force $::BDATA $b
    mwr -force $::CTRL 0x1
...
puts "--- A bit walk (b = 1 in both lanes) ---"
for {set k 0} {$k < 16} {incr k} {
    set x [expr {1 << $k}]
    set a [expr {$x | ($x << 16)}]
    set b [expr {1 | (1 << 16)}]
    lassign [both $a $b] g1 g2
    puts [format "  A bit %2d : lo %-11s (want %-11s) %s | hi %-11s (want %-11s) %s" \
        $k $g1 $x [expr {$g1==$x?"ok":"BAD"}] $g2 $x [expr {$g2==$x?"ok":"BAD"}]]
}
puts "--- B bit walk (a = 1 in both lanes) ---"
for {set k 0} {$k < 16} {incr k} {
    set x [expr {1 << $k}]
    set b [expr {$x | ($x << 16)}]
    set a [expr {1 | (1 << 16)}]
    lassign [both $a $b] g1 g2
    puts [format "  B bit %2d : lo %-11s (want %-11s) %s | hi %-11s (want %-11s) %s" \
        $k $g1 $x [expr {$g1==$x?"ok":"BAD"}] $g2 $x [expr {$g2==$x?"ok":"BAD"}]]
}
puts "--- small square table ---"
for {set x 0} {$x < 12} {incr x} {
    set a [expr {$x | ($x << 16)}]
    lassign [both $a $a] g1 g2
    puts [format "  %2d*%2d = %-6d : lo %-11s %s | hi %-11s %s" \
        $x $x [expr {$x*$x}] $g1 [expr {$g1==$x*$x?"ok":"BAD"}] \
        $g2 [expr {$g2==$x*$x?"ok":"BAD"}]]
}
exit 0
